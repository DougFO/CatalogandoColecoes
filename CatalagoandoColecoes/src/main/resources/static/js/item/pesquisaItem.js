...
function pesquisar() {
	...
	//if($("#opcaoPesquisa").val() == 'isbn') {
		if(document.querySelector('input[name=opcao]:checked').value == 'manga') {
			//console.log("mangaPesquisa");
			window.location.href = "/item/"+$("#lIsbnPesquisa").val()+"/manga";
		} else {
			//console.log("hqPesquisa");
			window.location.href = "/item/"+$("#lIsbnPesquisa").val()+"/hq";
		}
	...
	/*else {
		if($("#opcaoPesquisa").val() == 'titulo') {
			window.location.href = "/itensPesquisados/"+$("#lTituloPesquisa").val();
		}
	}*/
}